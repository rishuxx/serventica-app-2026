...
export class LocationValidationService {
  ...
  private readonly MAX_SPEED_MPS = 50;
  ...
  private readonly MAX_STALENESS_MS = 15000;

  /**
   * Validates partner coordinates, accuracy, staleness, and impossible physical teleportation.
   */
  public validatePartnerLocation(
    raw: Partial<PartnerLiveLocation>,
    lastKnown?: PartnerLiveLocation | null
    ...
    // Note: If lastKnown is older than 30s (e.g. initial session seed or gap),
    // this incoming fix is authoritative and establishes/resets partner position without being dropped.
    if (lastKnown && lastKnown.latitude != null && lastKnown.longitude != null) {
      ...
      // Only perform speed/jump validation if lastKnown was recent (<= 30 seconds ago)
      if (timeDiffSec <= 30 && timeDiffSec >= 0) {
        ...
        if (computedSpeed > this.MAX_SPEED_MPS && distMeters > 200) {
          this.logger.warn(
            `[JumpDetected] Booking ${bookingId}: jumped ${distMeters.toFixed(1)}m in ${safeTimeDiffSec}s (${computedSpeed.toFixed(1)} m/s)`
          );
          return { isValid: false, reason: 'IMPOSSIBLE_MOVEMENT_JUMP_DETECTED' };
        }
      } else {
        this.logger.log(
          `[AuthoritativeFixEstablished] Booking ${bookingId}: established new GPS anchor (${latitude.toFixed(6)}, ${longitude.toFixed(6)}) after ${timeDiffSec.toFixed(1)}s elapsed.`
        );
      }
    }

    // Sanitize and normalize heading [0, 360]
    let sanitizedHeading: number | undefined = undefined;
    if (raw.heading != null && typeof raw.heading === 'number' && !isNaN(raw.heading)) {
      ...
      if (dist >= 3) {
        sanitizedHeading = Math.round(
          this.calculateBearing(lastKnown.latitude, lastKnown.longitude, latitude, longitude)
        );
      } else {
    ...
    const sanitizedLocation: PartnerLiveLocation = {
      bookingId,
      partnerId,
