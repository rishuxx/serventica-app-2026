...
async function runFulfillmentTests() {
  ...
  const pe = new PricingEngineService();
  ...
  assert(
    reassign.success && reassign.status === 'SEARCHING_PARTNER',
    'Partner reassignment revoked old authority and returned to SEARCHING_PARTNER'
  );
